...
static const char *const TAG = "fujitsu_general_ar_je5.climate";
...
const uint8_t FUJITSU_GENERAL_STATE_MESSAGE_LENGTH = 15;
const uint8_t FUJITSU_GENERAL_MESSAGE_TYPE_STATE = 0xFC;

// Util messages - off & eco etc.
const uint8_t FUJITSU_GENERAL_UTIL_MESSAGE_LENGTH = 6;
...
const uint8_t FUJITSU_GENERAL_MESSAGE_TYPE_SWING = 0x6D;

// State header
const uint8_t FUJITSU_GENERAL_STATE_HEADER_BYTE0 = 0x08;
...
const uint16_t FUJITSU_GENERAL_HEADER_MARK = 3400;
const uint16_t FUJITSU_GENERAL_HEADER_SPACE = 1700;

const uint16_t FUJITSU_GENERAL_BIT_MARK = 425;
const uint16_t FUJITSU_GENERAL_ONE_SPACE = 1275;
const uint16_t FUJITSU_GENERAL_ZERO_SPACE = 425;

const uint16_t FUJITSU_GENERAL_TRL_MARK = 425;
...
void FujitsuGeneralClimate::control(const climate::ClimateCall &call) {
  ESP_LOGV(TAG, "Control call received");
  if (call.get_swing_mode().has_value() && *call.get_swing_mode() != this->swing_mode) {
    // If the swing mode is set, we need to transmit it separately
...
void FujitsuGeneralClimate::transmit_off_() {
  ...
  this->power_ = false;
  this->swing_mode = climate::CLIMATE_SWING_OFF;  // reset swing mode
}

void FujitsuGeneralClimate::transmit_swing_() {
  ESP_LOGV(TAG, "Transmit off");
  ...
  remote_state[3] = FUJITSU_GENERAL_COMMON_BYTE3;
  remote_state[4] = FUJITSU_GENERAL_COMMON_BYTE4;
  remote_state[5] = FUJITSU_GENERAL_MESSAGE_TYPE_SWING;
...
bool FujitsuGeneralClimate::on_receive(remote_base::RemoteReceiveData data) {
  ...
  switch (recv_message_type) {
    ...
    case FUJITSU_GENERAL_MESSAGE_TYPE_SWING:
  ...
  if (recv_message_type == FUJITSU_GENERAL_MESSAGE_TYPE_STATE) {
    ...
    calculated_checksum = recv_checksum;  // util messages don't have a checksum
  }
  ...
  else if (recv_message_type == FUJITSU_GENERAL_MESSAGE_TYPE_SWING) {
    ESP_LOGV(TAG, "Received swing message");
    this->swing_mode = this->swing_mode==climate::CLIMATE_SWING_OFF ? climate::CLIMATE_SWING_VERTICAL : climate::CLIMATE_SWING_OFF;
  }
